...
const RequireAuth = ({ children }) => {
    ...
    if (!user.emailVerified && user.providerData[0].providerId === 'password') {
        return <div className='text-center h-[90vh] flex flex-col justify-center items-center bg-violet-200'>
            <p className='text-6xl mb-8 font-semibold text-slate-700 '>OPPS ! </p>
            <p className='text-red-500 text-6xl font-semibold'>Your Email is not Verified !!!
            </p>
            <FontAwesomeIcon icon={faSadTear} className='text-slate-500 text-6xl my-8'></FontAwesomeIcon>
            <button onClick={handleVErification} className='py-4 px-4 my-4 text-white bg-orange-400 hover:bg-orange-700 rounded-3xl text-2xl w-1/3'>Send Verification Mail Again...</button>
